...
class CycleGANModel(BaseModel):
    ...
    def set_input(self, input):
        dir1 = self.opt.which_direction1 == 'dir1'
        dir2 = self.opt.which_direction1 == 'dir2'
    ...
    def test(self):
        ...
        fake_C = self.netG_B(fake_B)
        self.rec_A = self.netG_C(fake_C).data
        ...
        fake_B = self.netG_A(fake_A)
        self.rec_C = self.netG_B(fake_B).data
        #self.fake_A = fake_A.data
        self.fake_B = fake_B.data
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        #loss_D=self.gloss(pred_fake,pred_real,False)
        # backward
        loss_D.backward()
        return loss_D
    ...
    def backward_D_C(self):
        fake_C = self.fake_B_pool.query(self.fake_C)
    ...
    def backward_G(self):
        ...
        if lambda_idt > 0:
            ...
            idt_A = self.netG_A(self.real_C)
            loss_idt_A = self.criterionIdt(idt_A, self.real_B) * lambda_B * lambda_idt
            # G_B should be identity if real_A is fed.
            idt_B = self.netG_B(self.real_A)
            loss_idt_B = self.criterionIdt(idt_B, self.real_A) * lambda_A * lambda_idt
            # G_C should be identity if real_B is fed.
            idt_C = self.netG_C(self.real_B)
            loss_idt_C = self.criterionIdt(idt_B, self.real_C) * lambda_A * lambda_idt
        ...
        # GAN loss D_A(G_A(A))
        fake_B2 = self.netG_A(self.real_A)
        pred_fake = self.netD_B(fake_B2)
        loss_G_A = self.criterionGAN(pred_fake, True)
        fake_C = self.netG_B(fake_B2)
        pred_fake = self.netD_C(fake_C)
        loss_G_B = self.criterionGAN(pred_fake, True)
        

        # GAN loss D_B(G_B(B))
        fake_C2 = self.netG_B(self.real_B)
        pred_fake = self.netD_C(fake_C2)
        loss_G_B += self.criterionGAN(pred_fake, True)
        fake_A = self.netG_C(fake_C2)
        ...
        loss_G_C += self.criterionGAN(pred_fake, True)
        fake_B = self.netG_A(fake_A2)
        pred_fake = self.netD_B(fake_B)
        loss_G_A += self.criterionGAN(pred_fake, True)
        

        # Forward cycle loss for A
        rec_A = self.netG_C(fake_C)
	...
	#Forward cycle loss for C
        rec_C = self.netG_B(fake_B)
        loss_cycle_C = self.criterionCycle(rec_C, self.real_C) * lambda_C

        # combined loss
        loss_G = (loss_G_A + loss_G_B + loss_G_C+ loss_cycle_A + loss_cycle_B +loss_cycle_C)*0.8
        loss_G.backward()
	...
	fake_C2 = util.tensor2im(self.fake_C2)
        rec_C = util.tensor2im(self.rec_C)
        ret_visuals = OrderedDict([('real_A', real_A), ('fake_B2', fake_B2), ('fake_C', fake_C), ('rec_A', rec_A),
				('real_B', real_B), ('fake_C2', fake_C2), ('fake_A', fake_A), ('rec_B', rec_B),
				('real_C', real_C), ('fake_A2', fake_A2), ('fake_B', fake_B), ('rec_C', rec_C)
                                   ])
